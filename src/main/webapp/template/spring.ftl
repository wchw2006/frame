...
-->
<#macro form action name="" method="post">
	<div class="portlet-body form">
		<form action="${action}" name="${name}" method="${method}" class="form-horizontal">
			<div class="form-body">
			  <#nested>
		    </div>
	   </form>
...
<#--
 * Form  Group
 * @param label lebel描述值
 * @param type input的类型   如'text,password等'
 * @param id  input的 id值
 * @param name input的name值
 * @param lcm label的样式值(例值: col-md-3) 如果不设置值 则label是单独的一行 
 * @param incm  input的样式值(例值:col-md-4) 如果不设置 则是单独的一行
 * @param placeholder input的里的指示值
 * @param help input输入值的帮助信息，常显示在input下
-->
<#macro formG label type id lcm="" incm="" name=id placeholder="" help="">
  <div class="form-group">
	<label class="${lcm} control-label">${label}</label>
	<div class="${incm}">
		<input type="${type}" id="${id}" name="${name}" class="form-control" placeholder="${placeholder}">
		<#if help != "">
